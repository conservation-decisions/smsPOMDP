#' @export
sim = function(t, o, r, state_prior, Tmax, a, discount=0.95, size = 1){
  ...
  plot1 = plot(sim$df$time, sim$df$state, yaxt='n', 
               pch = 19, xlab = 'Time (years)',ylab = 'State', 
               ylim = c(0.9,2.1), xlim = c(-2,Tmax))
  ...
  plot3 =plot(sim$df$time, sim$df$obs, yaxt='n',
              pch = 19, xlab = 'Time (years)',ylab = 'Observation',
              ylim = c(0.9,2.1), xlim = c(-2,Tmax) )
  ...
  plot4 = plot(c(0:(Tmax-1)), sim$state_posterior$extant, type = 'l', xlab = 'Time (years)',ylab = 'Probabilities', ylim = c(0,1), xlim = c(-2,Tmax))
  lines(c(0:(Tmax-1)), sim$state_posterior$extinct, col = 'red', cex = size)
  legend('bottomleft',legend = c('Extant','Extinct'), col = c('black', 'red'), lty = 1,  bty = "n", cex = size)
