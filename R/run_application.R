  ...
  app <- shiny::shinyApp(
    ...
    server <- function(input, output, session){
      ...
      shiny::observeEvent(input$sim, {
        output$plot = shiny::renderPlot({TigerPOMDP::sim(p0(), pm(), d0(), d(), V(), Cm(), Cs(), state_prior(), Tmax(), a0(), disc = 0.95, size = 2)})
      })

      shiny::observeEvent(input$graph, {
        output$plot = shiny::renderPlot({TigerPOMDP::graph(p0(), pm(), d0(), d(), V(), Cm(), Cs(), disc = 0.95, size = 2)})
      })
    }
  )
  shiny::runApp(app)
